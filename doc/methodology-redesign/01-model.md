...
(`ProcessRoleKinds.Contains` в `MethodologyInstanceService.AssertProcessRoleSingletonAsync`) — переносим в поле.
...
> (`ProcessRoleKinds`/`Methodological`, `MethodologyInstanceService.AssertProcessRoleSingletonAsync`,
> сегодня одним и тем же
...
null-membership» доску (`TasksService.CreateBoardAsync`) — сегодня null-membership исторический
...
GateEnforcement(                        // [НОВОЕ] что сервер БЛОКИРУЕТ
    ...
    artifacts: bool = true              // reason И precondition сегодня оба жёсткие (`WorkflowEngine.Validate` / `RequirePreconditionArtifactsAsync`)
)
```
...
>     раньше проверки, когда нет предыдущего статуса (`TasksService.PersistReasonCommentsAsync`, комментарий в коде —
...
>     `RequirePreconditionArtifactsAsync` (ныне `GuardEngine.RequirePreconditionArtifacts`) —
>     родиться СРАЗУ в статусе,
...
>     (ветка `from is null` в `GuardEngine.RequirePreconditionArtifacts`) — результат зависит от
>     порядка, не от намерения автора.
...
> ориентации (сверено): `idea_spec` = ideas→spec (`IRelationStore.ListByKindAsync`), `issue_task` = intake→work,
...
> **Правило рождения в `Blocked` без блокера** (`RequireBlockersAsync`, ныне `GuardEngine.RequireBlockers`)
...
Effect(                                 // [есть]
    ...
    onLeave: string?,                   // [НОВОЕ] статус ВЫХОДА — сработать, когда узел ПОКИНУЛ этот
                                         ...
                                         // `TaskTransitionEffects.RunTransitionEffectsAsync`) — onLeave
                                         // даёт вторую точку
...
Delivery(requiredTypes: string[], defectTypes: string[], link: string)
                                        ...
                                        // "task_spec" (`ComputeSpecDeliveryAsync`) — автор методологии
...
1. **Gating-consumption ребра `blocks`** (потребить ребро при `set`; разблокировать, только когда
   снят ПОСЛЕДНИЙ блокер) зашита в исполнителе на литерале `"blocks"`
   (`TaskTransitionEffects.RunTransitionEffectsAsync`) — builtin-поведение слага, НЕ выразимое генериком `Effect`. Кастомный вид связи gating не
   получает; `blocks` остаётся builtin.
2. **Рождение/удержание в `blocksGate.status` без блокера** — STATE invariant, не транзишн-гейт
   (`RequireBlockersAsync`, ныне `GuardEngine.RequireBlockers`, сегодня литерал `"Blocked"`, гейтится
...
3. **Уход из `blocksGate.status` вручную закрывает входящие `blocks`-рёбра**
   (`TasksService.CloseBlocksOnLeaveAsync`) и **удаление узла-блокера освобождает
   цель** в `blocksGate.releaseTo` (`TaskTransitionEffects.RunDeleteEffectsAsync`, сегодня литерал
   `"InProgress"`).
...
| Legacy null-membership доска (`TasksService.CreateBoardAsync`) | Utility-слой: явное членство в проектном utility-наборе (§2a) |
...
| `Effect` срабатывает только на ВХОД (`TaskTransitionEffects.RunTransitionEffectsAsync`) | `Effect.onLeave` — вторая точка входа (§7) |
