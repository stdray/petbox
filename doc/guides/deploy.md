...
## Снятие gcdump с прод-контейнера (chiseled-образ, self-contained)

Финальный образ — `mcr.microsoft.com/dotnet/nightly/runtime-deps:...-chiseled`, self-contained.
Внутри контейнера **нет ничего**: ни шелла, ни `dotnet` CLI, ни `curl`/`wget`,
`DOTNET_DiagnosticPorts` не задан, диагностический сокет наружу не проброшен, debug-таргета в
Dockerfile нет. Обычные рычаги (`docker exec dotnet-gcdump collect`, проброшенный diagnostic
port) недоступны — единственный путь снять дамп с работающего прод-контейнера идёт через
host-side namespace-трюк. Рецепт ниже добыт клиентом ценой двух суток разбора и одного
уроненного хоста — если он снова понадобится, это не должно стоить так дорого второй раз.

**Порядок действий:**

1. **`dotnet-gcdump` запускается с ХОСТА**, не изнутри контейнера — внутри его и поставить
   некуда (нет `dotnet` CLI, нет package-менеджера).
2. **Найти сокет цели через `/proc`.** Diagnostic IPC-сокет процесса виден с хоста по пути
   `/proc/<hostPid>/root/tmp/dotnet-diagnostic-1-<...>-socket`, где `<hostPid>` — PID процесса
   `PetBox.Web` **в host-namespace** (`docker top petbox` или `ps aux | grep PetBox.Web` на
   хосте), `/root/` здесь — это `/` контейнера, спроецированный через `/proc/<pid>/root`.
3. **Bind-mount этого пути в контейнер не работает** — `runc` отвечает `invalid argument`.
   Так что путь не «пробросить сокет внутрь», а «зайти к сокету снаружи через `/proc`».
4. **Главная ловушка: PID 1 в своём namespace.** Приложение внутри контейнера — это PID 1
   его собственного PID-namespace. `dotnet-gcdump collect -p <hostPid>` (с host-PID) **молча
   возвращает пустой дамп** — инструмент резолвит PID в контексте своего процесса/namespace, а
   не в host-namespace, где `<hostPid>` живёт. Рабочий обход: сделать симлинк на найденный
   сокет с именем, которое `dotnet-gcdump` ожидает от **PID 1**, и попросить его именно про
   PID 1:
   ```bash
   ln -s /proc/<hostPid>/root/tmp/dotnet-diagnostic-1-<...>-socket \
         /tmp/dotnet-diagnostic-1-000000-socket
   dotnet-gcdump collect -p 1 -o /tmp/petbox.gcdump
   ```
5. **В `/tmp` контейнера копятся осиротевшие сокеты** — по одному на каждый OOM-рестарт
   процесса (container id при рестарте не меняется, поэтому они не самоочищаются между
   рестартами). Перед тем как строить путь для симлинка, выбрать **свежайший** сокет по mtime
   (`ls -lt /proc/<hostPid>/root/tmp/dotnet-diagnostic-*-socket`), а не первый попавшийся —
   иначе дамп снимется со старого, уже не текущего процесса.

**Предупреждение — читать до, а не после того как хост ляжет второй раз:**
`dotnet-gcdump` строит граф объектов управляемой кучи **в памяти своего же процесса**, а не
цели. На маленьком хосте это само по себе может спровоцировать OOM: замерено 775 МБ RSS
у `dotnet-gcdump` при ~400 МБ свободных на хосте на момент снятия — этого хватило, чтобы
уронить хост целиком (глобальный OOM убил и цель, и сам `dotnet-gcdump`). Снимать сырой
`.gcdump`-файл на проде можно (это просто копирование потока диагностических данных), но
**строить отчёт/граф (`dotnet-gcdump report`, GUI-анализ) — только на другой машине**, скопировав
файл прочь с прод-хоста первым делом.

**Ловушка при чтении `dotnet-gcdump report`, найденная постфактум:** колонка **«Object
Bytes»** в выводе `report` — это размер **ОДНОГО объекта** (среднее по размерной полосе), а
**не сумма** по строке. Строки в отчёте разбиты по размерным полосам (size buckets) одного и
того же типа, и `report` их не сворачивает. Чтобы получить реальный вес типа в куче, нужно
просуммировать `per_object_bytes × count` по **всем** полосам этого типа — прочитать колонку
как «итого» и есть та ошибка, которая в этом расследовании увела первичный анализ утечки не
туда.
