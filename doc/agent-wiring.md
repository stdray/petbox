...
| `petbox-wire doctor` | Gate (exit code is significant): resolves the live agent definition from the server (LKG cache / built-in default on miss), then runs `checkTruthfulness(definition, harness)` for every id in `HARNESS_IDS` and prints OK or each violation. Also reports definition drift against the built-in default (degradation — kit poorer than server — is informational; true divergence is flagged separately), skill-file drift (materialized vs. kit templates: in sync / behind / foreign-BLOCKED), the session-banner budget margin, and a tail of `~/.petbox/wire.log`. Network checks are skipped with an explicit reason when the server is unreachable. The local binding is *noted*, not required. |
