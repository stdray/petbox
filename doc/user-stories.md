...
Конкретные ежедневные сценарии работы с PetBox. Каждая стори описывает, что пользователь (сейчас — автор; в будущем — приглашённые друзья) реально делает: куда кликает, куда идёт, что видит.

Это источник правды для того, какие потоки должны работать end-to-end. E2E тесты в `tests/PetBox.E2ETests/` должны привязываться к одной или нескольким стори отсюда. Если в тестах есть поток, которого нет здесь — либо это баг в этом документе, либо тест переусложнён.

Каждая стори содержит:
- **Цель** — что пользователь хочет получить.
- **Триггер** — почему открыл PetBox прямо сейчас.
...
**Шаги:**
1. Открыл PetBox → приземлился на `/ui/{ws}` (Status overview). [Workspace резолвится из cookie/claim/membership.]
...
**Триггер:** "Twitter-бот не запостил ночью." Пользователь открывает PetBox.
...
**Цель:** завести пет-проект, которого ещё нет в PetBox.
...
9. **В коде пета:** настраивает обращение к `https://petbox/api/...` с новым ключом + service key.
...
**Результат:** агент работает против PetBox с bounded credentials и self-documented инструкциями.
...
E2E-тесты в `tests/PetBox.E2ETests/` мапятся к стори так:
