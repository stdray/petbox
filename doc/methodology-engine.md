...
{ name,                                   — slug определения
  ...
  linkKinds: [{ slug, description? }],    — проектные виды связей для relations.create
...
| work   | feature, bug, chore               | Pending, InProgress, Review, Blocked, Deferred, Done✓, Cancelled✕ | Review→Done — владелец; feature/bug — linkConstraint task_spec |
...
- **tagAxes** — enforced в `SetTagsAsync`/`TagStore`: оси объявлены → namespace тега должен
  быть из списка, «голые» теги отклоняются.
- **Словарь связей** — `relations.create` принимает: процессные виды (`task_spec`,
  ...
  без эффектов) и объявленные проектом `linkKinds` (тоже без эффектов). Неизвестный вид —
  отказ со списком допустимых. Валидация словаря — в сервисе
...
| Глагол | Что делает |
|--------|------------|
| `tasks.methodology_def_upsert` | записать определение (baseline-версия; опц. `migration`) |
| `tasks.methodology_def_get` | прочитать определение; без него — `defined:false, preset:"builtin-presets"` |
| `tasks.methodology_guide` | агентский onboarding-гайд, выведенный из данных: markdown + структурные инварианты (`approval_gate\|reason_required\|precondition_artifact\|link_constraint\|tag_axes`), source = presets\|definition\|mixed |
| `tasks.workflow` | FSM конкретной доски (кастомной или пресетной), переходы с `preconditionArtifact` |
| `tasks.board_create` | принимает и пресетные, и объявленные определением kind'ы |

Не путать: `tasks.methodology_get` — это ИНДЕКС квартетных досок (узлы/статусы), не
определение процесса.

`tasks.methodology_guide` — это «артефакты из данных» v1: скилл-текст процесса порождается
