...
public sealed class WalCheckpointServiceTests : IAsyncLifetime
{
	...
	{
		_baseDir = Path.Combine(Path.GetTempPath(), "yobabox-wal-test-" + Guid.NewGuid().ToString("N"));
	...
	public async Task RunOncePassAsync_TruncatesWal_OnActiveDb()
	{
		...
		using (var scope = _factory.Services.CreateScope())
		{
			...
			await db.InsertAsync(new YobaBox.Core.Models.Workspace { Key = "test", Name = "T", CreatedAt = DateTime.UtcNow });
			await db.Projects.Where(p => p.Key == TestProjectKey).DeleteAsync();
			await db.InsertAsync(new YobaBox.Core.Models.Project { Key = TestProjectKey, WorkspaceKey = "test", Name = "K" });
			await db.InsertAsync(new YobaBox.Core.Models.DataDb
			{
