...
public sealed class QueryExecApiTests : IAsyncLifetime
{
	...
	{
		_baseDir = Path.Combine(Path.GetTempPath(), "yobabox-qe-test-" + Guid.NewGuid().ToString("N"));
	...
	public async Task Query_TimeoutHeader_AcceptedWithinLimit()
	{
		...
		_client.DefaultRequestHeaders.Remove("X-Yobabox-Timeout-Seconds");
		_client.DefaultRequestHeaders.Add("X-Yobabox-Timeout-Seconds", "60");
		var resp = await _client.PostAsJsonAsync($"/api/data/{TestProjectKey}/{TestDbName}/query",
