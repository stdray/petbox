...
public sealed class LlmAdminPageTests : IDisposable
{
	...
	sealed class FakeEditor : ILlmRegistryEditor
	{
			...
			IReadOnlyDictionary<string, string> apiKeys, long version, CancellationToken ct = default)
		{
			...
			// Version is bumped by SaveAsync itself now (work llm-admin-page-no-cas moved it there so
			// the CAS check and the bump cannot drift apart) — PatchAsync must NOT bump it again.
