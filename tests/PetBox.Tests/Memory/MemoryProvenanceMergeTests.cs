...
public sealed class MemoryProvenanceMergeTests : IDisposable
{
	...
	public async Task CrossScope_StrongWorkspaceHit_OutranksWeakProjectHit()
	{
		...
		await MemoryTools.RememberAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, "deploy deploy deploy release", scope: "project");
		await MemoryTools.RememberAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, "deploy deploy pipeline", scope: "project");
		var pWeak = await MemoryTools.RememberAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, "a note that mentions deploy once", scope: "project");
		// Workspace: a single top-ranked hit.
		var wStrong = await MemoryTools.RememberAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, "deploy", scope: "workspace");

		var res = await MemoryTools.SearchAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, new NoopUsageRecorder(), "deploy");
	...
	{
		var http = Http("memory:read,memory:write");
		await MemoryTools.RememberAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, "kafka rebalance storm", scope: "project");
		await MemoryTools.RememberAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, "kafka rebalance storm", scope: "workspace");
	...
	public async Task SourcesCount_CountsDistinctSessionProvenance_OmitsWhenAbsent()
	{
		...
		var res = await MemoryTools.SearchAsync(http, Flags(), _db.Factory().WorkspaceMemory(), _memory, new NoopUsageRecorder(), "kubernetes", scope: "project");
