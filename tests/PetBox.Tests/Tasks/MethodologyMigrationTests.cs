...
public sealed class MethodologyMigrationTests : IAsyncLifetime
{
	...
	const string AgentKey = "yb_key_mmig_agent"; // tasks:read,tasks:write
	...
	public MethodologyMigrationTests()
	{
		_baseDir = Path.Combine(Path.GetTempPath(), "petbox-mmig-" + Guid.NewGuid().ToString("N"));
	...
	public async Task InitializeAsync()
	{
		...
		using (var scope = _factory.Services.CreateScope())
		{
			...
			await db.InsertAsync(new Project { Key = ProjectKey, WorkspaceKey = "test", Name = "Migration" });
