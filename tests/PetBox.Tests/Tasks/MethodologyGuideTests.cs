...
public sealed class MethodologyGuideTests : IAsyncLifetime
{
	...
	const string AgentKey = "yb_key_mgd_agent"; // tasks:read,tasks:write
	...
	public MethodologyGuideTests()
	{
		_baseDir = Path.Combine(Path.GetTempPath(), "petbox-mgd-" + Guid.NewGuid().ToString("N"));
	...
	public async Task InitializeAsync()
	{
		...
		using (var scope = _factory.Services.CreateScope())
		{
			...
			await db.InsertAsync(new Project { Key = ProjectKey, WorkspaceKey = "test", Name = "Guide" });
