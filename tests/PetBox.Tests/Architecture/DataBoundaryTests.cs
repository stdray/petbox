...
// Guard for the Data tier convergence: the data.query / data.exec tools must not
...
public sealed class DataBoundaryTests
{
	...
	public void DataTools_DoNotOpen_RawSqliteConnections()
	{
		...
		result.IsSuccessful.Should().BeTrue(
			"data.query/data.exec must run SQL through IDataSqlService, not a raw connection; offenders: "
