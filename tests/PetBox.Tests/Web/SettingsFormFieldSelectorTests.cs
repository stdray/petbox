...
public sealed class SettingsFormFieldSelectorTests
{
	...
	[Fact]
	public void BrowserState_AtUserScope_ShowsTheme_And_SearchRankingMode()
	{
		...
		// [Setting] properties. Theme (work `ui-state-theme-unify`) and SearchRankingMode (work
		// `ui-search-ranking-mode-preference`) are the two live [Setting] fields this page renders.
		var visible = SettingsFormFieldSelector.GetEditable(typeof(BrowserState), Scope.User);

		visible.Select(v => v.Property.Name).Should().BeEquivalentTo(
			[nameof(BrowserState.Theme), nameof(BrowserState.SearchRankingMode)]);
	}
