...
public sealed class DeployToolsTests : IDisposable
{
	...
	{
		var r = Json(await DeployTools.NodeUpsertAsync(Http("deploy:write"), Flags(), _svc, _db.Factory().AgentKeys(),
	...
	public async Task Upsert_Then_Stop_Start_Move()
	{
		...
		await DeployTools.NodeUpsertAsync(Http("deploy:write"), Flags(), _svc, _db.Factory().AgentKeys(), "n2");
