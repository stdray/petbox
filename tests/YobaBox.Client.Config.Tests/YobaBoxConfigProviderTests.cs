...
using YobaBox.Client.Config;

namespace YobaBox.Client.Config.Tests;

public class YobaBoxConfigProviderTests
{
	[Fact]
	public void Constructor_throws_when_BaseUrl_missing()
	{
		var options = new YobaBoxConfigOptions { ApiKey = "key" };
		var act = () => new YobaBoxConfigProvider(options);
	...
	{
		var options = new YobaBoxConfigOptions { BaseUrl = "https://yobabox.test" };
		var act = () => new YobaBoxConfigProvider(options);
	...
	public void Load_populates_data_from_fetched_json()
	{
		...
		using var provider = new YobaBoxConfigProvider(options);
		provider.Load();
