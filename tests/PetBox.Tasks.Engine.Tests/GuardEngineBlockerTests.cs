...
public sealed class GuardEngineBlockerTests
{
	...
	{
		var v = GuardEngine.RequireBlockers(Ctx(), [State("t1", "Blocked", "feature", nodeId: Id("t1"))], NoRefs);
	...
	public void BlockedTask_NamingABlockerInThisCall_Passes() =>
		GuardEngine.RequireBlockers(Ctx(), [State("t1", "Blocked", "feature", nodeId: Id("t1"))], Refs(("t1", Id("b1"))))
			.Should().BeNull();
	...
	public void BlockedTask_WithAnAlreadyActiveInboundBlocksEdge_Passes()
	{
		...
		GuardEngine.RequireBlockers(ctx, [State("t1", "Blocked", "feature", nodeId: Id("t1"))], NoRefs).Should().BeNull();
	}
	...
	public void BlockedTask_WithAnEmptyEdgeList_IsRefused()
	{
		...
		GuardEngine.RequireBlockers(ctx, [State("t1", "Blocked", "feature", nodeId: Id("t1"))], NoRefs)!.Node.Should().Be("t1");
	}
	...
	public void NonBlockedStatuses_AreNotJudged()
	{
		foreach (var s in new[] { "Pending", "InProgress", "Review", "Done", "Cancelled" })
			GuardEngine.RequireBlockers(Ctx(), [State("t1", s, "feature", nodeId: Id("t1"))], NoRefs).Should().BeNull();
	}
	...
	public void NonWorkKinds_AreNotJudged()
	{
		...
		GuardEngine.RequireBlockers(ctx, [State("i1", "Blocked", "idea", nodeId: Id("i1"))], NoRefs).Should().BeNull();
	}
	...
	public void WorkKind_IsDetectedOnADEFINEDKindToo_NotOnlyOnTheBarePreset()
	{
		...
		foreach (var runtime in new[] { Quartet, Presets })
			GuardEngine.RequireBlockers(Ctx(runtime: runtime), [State("t1", "Blocked", "feature", nodeId: Id("t1"))], NoRefs)
