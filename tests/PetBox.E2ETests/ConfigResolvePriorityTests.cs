...
public sealed class ConfigResolvePriorityTests(WebAppFixture app, ITestOutputHelper output) : IAsyncLifetime
{
	...
	[Fact(Skip = "Bindings created via Editor UI don't resolve in this shared-DB test — Editor save/redirect flow under rework. Subset-semantics algorithm is unit-tested in PetBox.Tests.Config.ResolvePipelineTests; cross-UI flow is covered by KpVotesOnboardingTests.S4 which creates bindings via API.")]
