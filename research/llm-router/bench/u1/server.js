...
function sseChatShellCall(res, model, toolName) {
  res.writeHead(200, { 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', Connection: 'keep-alive' });
  const id = 'chatcmpl-shell-' + Date.now();
  const base = { id, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model };
  const send = (delta, finish) => res.write(`data: ${JSON.stringify({ ...base, choices: [{ index: 0, delta, finish_reason: finish }] })}\n\n`);
  const argsStr = JSON.stringify({ cmd: 'echo hi' });
  send({ role: 'assistant', content: null, tool_calls: [{ index: 0, id: 'call_shell_1', type: 'function', function: { name: toolName, arguments: '' } }] }, null);
  send({ tool_calls: [{ index: 0, function: { arguments: argsStr } }] }, null);
  send({}, 'tool_calls');
  res.write('data: [DONE]\n\n');
  res.end();
}

function jsonChatShellCall(res, model, toolName) {
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({
    id: 'chatcmpl-shell-' + Date.now(), object: 'chat.completion', created: Math.floor(Date.now() / 1000), model,
    choices: [{ index: 0, message: { role: 'assistant', content: null, tool_calls: [{ id: 'call_shell_1', type: 'function', function: { name: toolName, arguments: JSON.stringify({ cmd: 'echo hi' }) } }] }, finish_reason: 'tool_calls' }],
...
const server = http.createServer(async (req, res) => {
  ...
  if (process.env.U1_DUMP_BODIES) {
    const dumpDir = process.env.U1_DUMP_BODIES;
    fs.mkdirSync(dumpDir, { recursive: true });
    fs.writeFileSync(path.join(dumpDir, `req-${Date.now()}-${Math.random().toString(36).slice(2, 7)}.json`), body);
  }
  ...
  if (req.url.startsWith('/v1/chat/completions')) {
    ...
    // If the caller offers an exec/shell-shaped tool (codex's exec_command), call it for real
    // with `echo hi` so B1 proves a genuine one-shell-call round trip, not just a canned tool name.
    const tools = parsed.tools || [];
    const shellTool = tools.find((t) => t.function && /exec_command|shell|exec/i.test(t.function.name));
    if (parsed.stream) {
      if (isToolResult) sseChatText(res, model, `shell said: ${String(last.content).slice(0, 60)}`);
      else if (shellTool) sseChatShellCall(res, model, shellTool.function.name);
      else sseChatToolCall(res, model);
    } else {
      if (isToolResult) jsonChatText(res, model, `shell said: ${String(last.content).slice(0, 60)}`);
      else if (shellTool) jsonChatShellCall(res, model, shellTool.function.name);
