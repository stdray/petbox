...
фильтром, не роллапящийся, ничего не архивирующий при закрытии. Доска `roadmap` (ниже) когда-то
несла ту же Phase→Wave→Task форму, но **(уточнено владельцем 2026-09-25)** она не была
работающим механизмом, который попробовали и бросили — это разовый импорт файла-плана из
`doc/` на диске; к моменту, когда задачи стали жить полноценно (quartet), импорт уже был
устаревшим сам по себе. Урока «не повторить чужую ошибку» отсюда нет — вопрос по существу
«изобрести», не «понять, почему бросили».
...
`roadmap` несла ту же Phase→Wave→Task форму (через `partOf` на отдельной simple-доске), и
сама доска сейчас `closed:true`, то есть «закрыть этап» когда-то буквально означало «закрыть
доску целиком». **(уточнено владельцем 2026-09-25)** Это не «прошлое решение D5», принятое и
затем брошенное: `roadmap` — разовый импорт плана из `doc/`-файла, устаревший ещё до того, как
задачи стали жить полноценно на quartet. Причины ЦЕЛЕНАПРАВЛЕННОГО отказа от паттерна искать
не нужно — её не было, доска просто перестала обновляться вместе с файлом, из которого её
когда-то импортировали.
...
семантически не определён; не решает D4 без отдельного архив-механизма. Риска «повторить
судьбу `roadmap`» в смысле «паттерн попробовали и бросили» нет **(уточнено владельцем
2026-09-25: `roadmap` — устаревший импорт, не брошенный эксперимент)**; остаётся обычный
риск нового primitive — что B тоже не будет поддерживаться, если ничто не обязывает агента
его использовать (см. §4).
...
- ~~Прежде чем строить B или C, стоит понять, ПОЧЕМУ `roadmap` бросили при переходе на
  quartet~~ — снято **(уточнено владельцем 2026-09-25)**: `roadmap` не мигрировали, потому что
  её было нечего мигрировать как живой механизм — это устаревший импорт файла-плана из `doc/`,
  не Phase/Wave/Task-эксперимент, который вели и свернули. Вопрос больше не блокирует B/C.
