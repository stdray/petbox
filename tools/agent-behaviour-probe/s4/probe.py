...
# This S4 harness lives one level DEEPER than the original probe (tools/agent-behaviour-probe/s4)
# because `.gitignore` ignores `tools/*` and un-ignores only `tools/agent-behaviour-probe/`: a
# sibling directory would have been invisible to git and the S4 leg unreproducible.
...
# ---------------------------------------------------------------- S4: owner-only skills block
#
# The fix under test (work user-invocable-skills-invisible-to-model) does NOT ship inside a
# SKILL.md. It is a system-prompt block the kit builds at session start:
#   * Claude Code -- pull-memory.ts, SessionStart hook -> session banner;
#   * opencode    -- opencode-plugin.ts, experimental.chat.system.transform -> output.system.
# BOTH of those injection sites are gated on `resolveProject(cwd)` being non-null, i.e. on the
# directory being REGISTERED in ~/.petbox/projects.json. The probe workspace is deliberately
# UNREGISTERED (containment item 4: a registered workspace would have the global Stop hook
# mirror probe sessions into a real project). So in the M0-shaped workspace the fix cannot
# reach the agent at all, and a run there measures the absence of the fix, not the fix.
#
# Resolution: render the block with the KIT'S OWN function (buildOwnerOnlySkillsBlock via
# emit-block.mjs -- never a Python re-implementation, which would measure text this probe
# wrote) and deliver it through each harness's system-prompt channel by hand. Both conditions
# are run: --owner-only-block on (the shipped state, in a registered project) and off (the
# control). The deviation from the real channel is recorded in provenance.json.
OWNER_ONLY = {"enabled": False, "claude": "", "opencode": "", "delivery": {}}


def render_owner_only_blocks(ws: Path) -> dict:
    """Render both harness variants of the block by calling the kit's own TypeScript."""
    out = {}
    for key, harness in (("claude", "claude-code"), ("opencode", "opencode")):
        p = subprocess.run(["node", str(HERE / "emit-block.mjs"), str(ws), harness],
                           capture_output=True, timeout=120, shell=False)
        txt = p.stdout.decode("utf-8", "replace")
        if p.returncode != 0 or not txt or txt == "<<NULL>>":
            raise RuntimeError(f"owner-only block render failed for {harness}: "
                               f"rc={p.returncode} {p.stderr.decode('utf-8', 'replace')[:300]}")
        out[key] = txt
    return out
...
def build_workspace(dest: Path, project: str, workspace: str, key_env: str) -> list[str]:
    ...
    if OWNER_ONLY["enabled"]:
        blocks = render_owner_only_blocks(dest)
        OWNER_ONLY.update(blocks)
        # opencode: the EXACT shipped channel -- a workspace plugin pushing onto output.system
        # from experimental.chat.system.transform, which is what opencode-plugin.ts does. Both
        # `.opencode/plugin/` and `.opencode/plugins/` were measured to load here (marker file
        # written from the hook); only one is used, so the block is pushed exactly once.
        plug = oc / "plugin"
        plug.mkdir(parents=True, exist_ok=True)
        (plug / "probe-owner-only.js").write_text(
            "// Probe-only shim. Reproduces opencode-plugin.ts's injection site for a workspace\n"
            "// that is deliberately NOT registered in ~/.petbox/projects.json (the real plugin\n"
            "// no-ops on an unregistered directory: `if (!resolved) return;`). The TEXT is the\n"
            "// kit's own buildOwnerOnlySkillsBlock output, not a re-write.\n"
            "const BLOCK = " + json.dumps(blocks["opencode"], ensure_ascii=False) + ";\n"
            "export const ProbeOwnerOnly = async () => ({\n"
            "  \"experimental.chat.system.transform\": async (_input, output) => {\n"
            "    output.system.push(BLOCK);\n"
            "  },\n"
            "});\n"
            "export default ProbeOwnerOnly;\n", encoding="utf-8")
        OWNER_ONLY["delivery"] = {
            "claude": "--append-system-prompt (real channel is pull-memory.ts SessionStart "
                      "additionalContext, which no-ops on an unregistered workspace)",
            "opencode": ".opencode/plugin/probe-owner-only.js -> "
                        "experimental.chat.system.transform -> output.system.push "
                        "(same API call as opencode-plugin.ts, minus the registry gate)",
        }
    return skills
...
def run_claude(ws: Path, prompt: str, env: dict, timeout: int) -> tuple[str, str, int, float]:
    ...
    if OWNER_ONLY["enabled"] and OWNER_ONLY["claude"]:
        cmd += ["--append-system-prompt", OWNER_ONLY["claude"]]
...
def cmd_run(args):
    ...
    ws = Path(args.ws).resolve()
    OWNER_ONLY["enabled"] = bool(args.owner_only_block)
    ...
    prov["owner_only_block_condition"] = "on" if OWNER_ONLY["enabled"] else "off (control)"
    prov["owner_only_block_delivery"] = OWNER_ONLY["delivery"]
    prov["owner_only_block_text"] = {k: OWNER_ONLY[k] for k in ("claude", "opencode")
                                     if OWNER_ONLY[k]}
    prov["owner_only_block_sha256"] = {k: sha(OWNER_ONLY[k]) for k in ("claude", "opencode")
                                       if OWNER_ONLY[k]}
...
def main():
    ...
    ap.add_argument("--owner-only-block", action="store_true",
                    help="deliver the kit's owner-only-skills block through each harness's "
                         "system-prompt channel (the shipped state in a REGISTERED project); "
                         "omit for the control condition")
    a = ap.parse_args()
