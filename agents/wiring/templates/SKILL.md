...
required on `memory.upsert`; `tags` is free CSV. `memory.recall` is the cheap hybrid entry
point (use `bodyLen`); `memory.search` is FTS5-ranked.
...
- `memory.store_list / store_create / store_delete / list / search / recall / remember / get / upsert / delta`
