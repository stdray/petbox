...
function rolesWith(
  ...
  const profiles: Record<string, { agents: Record<string, { roles: Record<string, { model: string }> }> }> = {
    default: {
      agents: { codex: { roles: Object.fromEntries(Object.entries(roles).map(([r, m]) => [r, { model: m }])) } },
    },
  };
  if (opts.other) {
    profiles["alt"] = {
      agents: {
        codex: { roles: Object.fromEntries(Object.entries(opts.other).map(([r, m]) => [r, { model: m }])) },
      },
    };
  }
  return { activeProfile: opts.activeProfile ?? "default", profiles };
}
