...
test("collectCodexRoleModelSlugsFromData stays UNION-wide: model-registration-check.ts's codex branch depends on it (do not narrow)", () => {
  const data: RolesFile = {
    activeProfile: "default",
    profiles: {
      default: { agents: { codex: { roles: { orchestrator: { model: "deepseek-v4-pro" } } } } },
      alt: { agents: { codex: { roles: { reserve: { model: "grok-4.6" } } } } },
    },
  };
    ...
    saveRoles(
      {
        ...
        profiles: {
          default: {
            agents: {
              codex: {
                roles: { orchestrator: { model: "deepseek-v4-pro" }, reserve: { model: "grok-4.6" } },
              },
            },
          },
        },
