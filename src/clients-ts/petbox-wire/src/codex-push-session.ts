...
async function main(): Promise<void> {
  try {
    ...
    try {
      ...
      // A SessionEnd hook has no additionalContext-style channel back into the session it just
      // ended, so stderr plus registry.ts's own wire.log trace is the loudest this hook can be.
