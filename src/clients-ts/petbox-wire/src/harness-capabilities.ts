 ...
 *   spawn_subagents — can spawn subagents (agent types / Task-equivalent). Measured live
 *     on opencode 2.0.12 (work: opencode-v2-background-subagents-remeasure, comment tag
 *     `measurement`; card: opencode-v2-background-subagent-still-opt-in-not-default;
 *     fix card: harness-capabilities-opencode-v2-background-claim-correction) — an
 *     EARLIER reading of the v1→v2 migration guide (opencode.ai/v2/docs/migrate-v1/,
 *     "Commands" section: "delegated commands now run automatically in the background")
 *     claimed this became background BY DEFAULT ("native, not opt-in"). That is wrong:
 *     the live tool is named `subagent` (`subtask` is a deprecated alias per the live
 *     OpenAPI schema) and runs in the FOREGROUND — blocking for the child's full duration
 *     — unless the call passes `background: true`. What IS true: the v1 env flag
 *     `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS` is obsolete on v2 — no flag gates the
 *     capability any more (the kit never rendered that flag itself, so there is nothing
 *     here to remove) — but "flag obsolete" and "opt-in by default" are two different
 *     facts; do not re-collapse them. Other v2 deltas from the same measurement: parent
 *     abort no longer cascades to the child (v1 killed it; v2's parent goes idle the
 *     instant `background:true` returns, so there is nothing left to interrupt); delivery
 *     is a first-class `type:"synthetic"` message (`metadata:{source:"subagent",
 *     childID, …}`, tag `<subagent …>`), replacing v1's `synthetic:true` user message
 *     tagged `<task …>`; a background child has no `subagent` tool in its own catalog and
 *     so cannot nest — whether that is `experimental.subagent_depth` enforcement or a bare
 *     mode restriction was NOT isolated, left unchecked rather than guessed. No new
 *     capability id: `spawn_subagents` was already true for opencode and stays true —
 *     backgrounding is a per-call argument under the same capability, not a new one.
 *
