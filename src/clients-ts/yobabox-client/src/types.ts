...
/** Options for YobaBoxConfigClient. */
export interface YobaBoxConfigClientOptions {
	/** Base URL of the YobaBox server, e.g. "https://yobabox.3po.su". Trailing slash optional. */
...
/** Structured error from the YobaBox config API. */
export class YobaBoxConfigError extends Error {
	readonly status: number;
	readonly body: unknown;

	constructor(message: string, status: number, body: unknown) {
		super(message);
		this.name = "YobaBoxConfigError";
		this.status = status;
		this.body = body;
	}
}

/** Events emitted by YobaBoxConfigClient. */
export interface YobaBoxConfigClientEvents {
