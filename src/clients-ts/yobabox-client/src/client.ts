...
import {
	...
	type Template,
	TypedEmitter,
	type YobaBoxConfigClientEvents,
	type YobaBoxConfigClientOptions,
	YobaBoxConfigError,
} from "./types.js";
...
/**
 * TypeScript SDK client for YobaBox config.
 *
 ...
 *   endpoint: 'https://yobabox.3po.su',
 *   apiKey: process.env.YOBABOX_API_KEY!,
 ...
 * ```
 */
export class YobaBoxConfigClient extends TypedEmitter<YobaBoxConfigClientEvents> {
	...
	constructor(options: YobaBoxConfigClientOptions) {
		super();
	...
	private ensureNotDisposed(): void {
		if (this.disposed) throw new Error("YobaBoxConfigClient is disposed");
	}
	...
	private async fetchOnce(): Promise<ResolvedConfig> {
		...
		try {
			...
			throw new YobaBoxConfigError(`Failed to reach YobaBox at ${url}: ${String(cause)}`, 0, null);
		}
		...
		if (!response.ok) {
			...
			throw new YobaBoxConfigError(this.errorMessage(response.status, parsed), response.status, parsed);
		}
 ...
 * Same as `new YobaBoxConfigClient(opts).fetch()`.
 */
export const fetchConfig = (options: YobaBoxConfigClientOptions): Promise<ResolvedConfig> =>
	new YobaBoxConfigClient(options).fetch();
