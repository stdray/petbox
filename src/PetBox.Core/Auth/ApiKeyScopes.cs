...
public static class ApiKeyScopes
{
	...
	[
		...
		new(MethodologyWrite, "Govern the live process", "GOVERNANCE: any act over an EXISTING live process — rewrite its rules (methodology_rules_upsert), author a live rules document (methodology_create), move an existing board under other rules (board_adopt), retire the whole process (methodology_close), or add/retire/destroy/rewire one of its process-role boards (board_close, board_reopen, board_delete, board_set_spec). tasks:write writes nodes UNDER the process; this changes the process itself. NOT needed for inert templates (template_upsert/snapshot/delete — documents, not processes) or for board_create (constrained BY the rules, changes nothing that exists). Meaningless without tasks:write.", "Tasks"),
