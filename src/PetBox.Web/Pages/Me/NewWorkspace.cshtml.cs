...
// granted) is denied the GET. That is NOT the enforcement — the policy also runs on the POST, and
// WorkspaceProvisioning.CreateAsync re-checks the quota against the DB before it writes. A hidden
// button is a UI courtesy; the number is the gate.
[Authorize(Policy = "CanCreateWorkspace")]
public sealed class NewWorkspaceModel : PageModel
{
	readonly ICoreDbFactory _f;

	public NewWorkspaceModel(ICoreDbFactory f) => _f = f;
	...
	public async Task<IActionResult> OnPostAsync(string? key, string? name, string? description)
	{
		...
		var result = await WorkspaceProvisioning.CreateAsync(
			db, key, name, description, creator, bypassQuota: isSysAdmin, HttpContext.RequestAborted);
