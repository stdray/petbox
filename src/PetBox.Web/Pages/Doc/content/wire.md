...
- **`disable-model-invocation: true`** — the key Claude Code and Droid honour to refuse a *model-initiated* call, so the skill only runs when a human asks for it by name. Carried by `petbox-agent-factory`, `petbox-analysis-workspace` and `petbox-factory-run`. `petbox-card-check` deliberately does **not** carry it: it stays out of the digest yet remains callable by an agent that decides it applies.
