...
- **Data** — per-project SQLite databases with a REST raw-SQL pass-through (`/api/data/{project}/{db}/exec` and `/query`) and schema apply; also exposed over MCP (`db_*`, `data_*`) and the client libraries.
...
Go straight to REST — `/v1/conf` (config, ETag-aware), `/api/data/{project}/{db}/query|exec` (data), `/api/ingest/.../clef` or the OTLP endpoints (logs), `/v1/chat/completions` (LLM router, already OpenAI-compatible). All of it needs only the `X-Api-Key` header. Don't hand-guess routes and shapes — the full machine-readable surface is one GET away: **[`{{origin}}/openapi/v1.json`]({{origin}}/openapi/v1.json)** (see "OpenAPI" below). Feed it to your language's codegen (`openapi-generator`, `swagger-codegen`, …) for a typed client in minutes.
...
The REST surface has a live OpenAPI 3 document at **[`/openapi/v1.json`]({{origin}}/openapi/v1.json)** — served by the running instance itself, in every environment (including production, like this one), and generated from the actual routing table, so it can never drift from the code that's answering your requests. Anonymous, same as `/health`/`/version`: the document only describes the API *surface* (routes, request/response shapes), which is not a secret — every endpoint it lists still enforces its own auth (API key / cookie) exactly as before. This is the recommended starting point if you're working against a running PetBox with no repo access, especially for a language with no client SDK (see above) — point your codegen of choice at it. The same document is also checked into the repo at `doc/api/PetBox.Web.json` (regenerated at build time), for offline reading or diffing; if the two ever disagree, the live endpoint is authoritative. Failing both, fall back to the module descriptions on this page or the `*Api.cs` endpoint files (e.g. `src/PetBox.Config/ConfigApi.cs`, `src/PetBox.Data/QueryExecApi.cs`, `src/PetBox.Log.Core/LogApi.cs`).
