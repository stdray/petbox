...
// Edges bind to the stable TaskNode.NodeId, so they survive renames. Node refs are
...
public static class RelationTools
{
		...
		RelationCreateItemInput[]? items, string? kind, string? from, string? to)
	{
		...
		// `items` explicitly sent as [] (not omitted) with no single-form fallback is an empty
		// batch — almost always a client bug (a filter emptied the list, the call still went out).
		// Reject it with the same unified wording every batch write verb uses, rather than falling
		// through to CreateAsync where an empty resolved[]/conflicts[] would silently report
		// applied:false with no conflict to explain why.
