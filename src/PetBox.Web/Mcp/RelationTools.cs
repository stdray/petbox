...
public static class RelationTools
{
	...
	[Description("List relations touching a node id. direction ∈ from|to|both (default both). Use direction=to to find edges pointing AT a node (reverse traversal, e.g. which tasks implement a spec node). Requires tasks:read.")]
	public static Task<object> ListAsync(
		IHttpContextAccessor http, FeatureFlags features, IRelationStore relations,
		string projectKey, string nodeId, string? direction = null,
		CancellationToken ct = default) => ModuleMcp.GuardAsync(async () =>
	{
		ModuleMcp.AssertFeature(features, Feature.Tasks);
		ModuleMcp.AssertProject(http, projectKey);
		ModuleMcp.AssertScope(http, ApiKeyScopes.TasksRead);
		var list = await relations.ListAsync(projectKey, nodeId, direction ?? "both", ct);
		return (object)new { relations = list.Select(r => new { r.Id, r.Kind, r.FromNodeId, r.ToNodeId }).ToList() };
	});
