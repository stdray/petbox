...
// IConfigurationProvider that fetches resolved YobaBox config over HTTP on initial
...
public sealed class YobaBoxConfigProvider : ConfigurationProvider, IDisposable
{
	readonly YobaBoxConfigOptions _options;
	...
	public YobaBoxConfigProvider(YobaBoxConfigOptions options)
	{
	...
	async Task PollSafeAsync()
	{
		...
		catch (Exception)
		{
			...
			System.Diagnostics.Debug.WriteLine("YobaBoxConfig poll failed; keeping last-known-good data.");
		}
		finally
		{
			_pollGate.Exit();
