...
// A pure projection of PetBox.Tasks.Data.TaskNode carrying only the fields the guard/
...
// (NodeId/Status/Type). TaskNode itself can't be referenced from this assembly — its
// `using LinqToDB.Mapping` would drag linq2db into the DB-free engine — so the service
// maps TaskNode -> NodeState at the IO boundary instead of fluent-mapping TaskNode into
// a clean type.
//
...
// slice-3 solver is shown to read it off TaskNode — don't pre-guess the shape.
